...
_tzmx = timezone("America/Mexico_City")

try:
    from OpenSSL import crypto
except ImportError:
    _logger.warning(
        "OpenSSL library not found. If you plan to use l10n_mx_edi, please "
        "install the library from https://pypi.python.org/pypi/pyOpenSSL"
    )
...
def str_to_datetime(dt_str, tz=_tzmx):
    return tz.localize(fields.Datetime.from_string(dt_str))


class CertificatePayroll(models.Model):
    _name = "l10n_mx_payroll_edi.certificate"
    ...
    def _certify_and_stamp(
        self, xml_content_str, xslt_path, no_cert_attrib_name="NoCertificado"
    ):
        """Appends the seal stamp, certificate, and serial number to CFDI
        documents
        :param xml_content_str: The XML document string to certify and stamp
        :param xslt_path: Path to the XSLT used to generate the cadena chain
               (pipe delimited string of important values)
        :param no_cert_attrib_name: string of the NoCertificado (default)
               attribute which can be replaced with noCertificado
        :return: A string of the XML with appended attributes: NoCertificado,
                 Certificado, Sello
        """
        ...
        for record in self:
            ...
            try:
                ...
                raise ValidationError(
                    _("The certificate content is invalid " "%s.", e)
                ) from e
    ...
    def _unlink_except_invoices(self):
        ...
        if (
            ...
            raise UserError(
                _(
                    "You cannot remove a certificate if at least one invoice "
                    "has been signed. "
                    "Expired certificates will not be used as Odoo uses the "
                    "latest valid certificate. "
                    "To not use it, you can unlink it from the current company "
                    "certificates."
                )
            )
